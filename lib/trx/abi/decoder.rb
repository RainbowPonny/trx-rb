...
module Trx
  module Abi
    class Decoder
      ...
      def decode_address(value, _ = nil, start) # rubocop:disable Style/OptionalArguments
      ...
      def decode_static_bytes(value, subtype = nil, start = 0)
        trim(value, start, subtype.to_i * 8).scan(/.{2}/).map(&:hex).pack("C*").strip
      end

      def decode_dynamic_bytes(value, start = 0)
        location = decode_uint(value[start..(start + 63)]) * 2
        size = decode_uint(value[location..location + 63]) * 2
        value[location + 64..location + 63 + size].scan(/.{2}/).map(&:hex).pack("C*")
      end
